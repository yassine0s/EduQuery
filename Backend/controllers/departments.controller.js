...
exports.get_one =  async (req, res, next) => {
    let qry = await execQuery('SELECT * FROM `departments` WHERE id = ?;', [req.params.id]);
    if(qry.error) 
        return next(qry.error);
...
exports.create = async (req,res,next)=> {
        ...
        if(!req.body.name){
            return res.status(500).send({message: "department details in the body are missing!"});
        }
    
        /* Check if the department already exists */
        let qry = await execQuery('SELECT * FROM `departments` WHERE name=?;', [req.body.name]);
        if(qry.error)
            return next(qry.error);
        if(qry.results.length > 0)
...
let qry = await execQuery('DELETE FROM `departments` WHERE id=?', [req.params.id])
if(qry.error)
return next(qry.error)

return res.status(200).send(res.results[0], {message: "department deleted successfully"})
